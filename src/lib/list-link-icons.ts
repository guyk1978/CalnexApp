...
const ICON_PATHS: Record<LinkIconKind, string> = {
...
export function classifyLinkIcon(url: string, title = ""): LinkIconKind {
  ...
  if (u.includes("loan-calculator") || u.includes("/tools/loan") || (t.includes("loan") && !t.includes("student")))
    return "loan";
  if (t.includes("interest")) return "interest";
  return "default";
}

export function linkIconSvg(kind: LinkIconKind): string {
