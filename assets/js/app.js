(function () {
  ...
  const BLOG_CATEGORY_DESCRIPTIONS = {
    "Auto Loans":
      "Practical guides on financing, refinancing, affordability, and negotiating better terms for your next vehicle.",
    "Business Operations":
      "Frameworks for small-business cash flow, automation, and operational efficiency beyond personal borrowing.",
    "Home Equity":
      "Clear comparisons of HELOCs, home equity loans, and cash-out options—with lien position and payment risk explained.",
    "Interest Rates":
      "In-depth guides on understanding rate fluctuations, APR, ARM caps, and how market shifts affect your payments.",
    "Loan Basics":
      "Foundational explainers on amortization, interest math, and how different loan structures change total cost.",
    "Mortgage":
      "Step-by-step coverage of qualification, PMI, underwriting, and the decisions that shape your home loan.",
    "Mortgage Costs":
      "Transparent breakdowns of APR, points, PMI, closing costs, and how to compare true borrowing cost.",
    "Mortgage Strategy":
      "Strategic playbooks for fixed vs ARM, refinancing, prepayment, and long-horizon mortgage planning.",
    "Personal Loans":
      "Guides on consolidation, fixed vs variable rates, and when personal loans improve—or worsen—your position.",
    "Repayment Strategy":
      "Evidence-based frameworks for extra payments, payoff vs investing, and shortening amortization safely.",
    "Term Planning":
      "Comparisons of loan terms, payment trade-offs, and when shorter or longer schedules actually win."
  };

  const getBlogCategoryDescription = (category) =>
    BLOG_CATEGORY_DESCRIPTIONS[category] ||
    `Expert guides and practical frameworks on ${String(category || "this topic").toLowerCase()} for smarter borrowing decisions.`;

  const mergeBlogPosts = (primary = [], secondary = []) => {
  ...
  const normalizeBlogCategory = (value = "") => String(value).trim().toLowerCase();

  const postMatchesCategory = (post, category) =>
    normalizeBlogCategory(post.category) === normalizeBlogCategory(category);
  ...
  const blogTile = (post) => {
    ...
    const meta = [formatBlogDate(post.updatedDate), post.readTime].filter(Boolean).join(" · ");
    const img = post.image || post.thumbnail || post.heroImage;
    const thumb = img
      ? `<div class="cn-blog-card__thumb cn-blog-card__thumb--${accent}"><img src="${img}" alt="" loading="lazy" decoding="async" /></div>`
      : `<div class="cn-blog-card__thumb cn-blog-card__thumb--${accent}" aria-hidden="true"></div>`;
    return `
          <article class="cn-blog-card">
            <a href="/blog/${post.slug}/" class="cn-blog-card__link">
              ${thumb}
              <div class="cn-blog-card__body">
                ${meta ? `<p class="cn-blog-card__meta">${meta}</p>` : ""}
                <h3 class="cn-blog-card__title">${post.title}</h3>
                ${excerpt ? `<p class="cn-blog-card__excerpt">${excerpt}</p>` : ""}
                <p class="cn-blog-card__category">${category}</p>
              </div>
            </a>
          </article>`;
  };
  ...
  const renderBlogIndex = async () => {
    ...
    const defaultView = document.getElementById("cnBlogDefaultView");
    const categoryView = document.getElementById("cnBlogCategoryView");
    const categoryTitle = document.getElementById("blogCategoryTitle");
    ...
    try {
      ...
      if (embedded && embedded.textContent.trim()) {
        embeddedPosts = JSON.parse(embedded.textContent);
      }
      let remotePosts = [];
      try {
        remotePosts = await fetchJson(asset("/data/blog.json"));
      } catch (_remoteError) {
        remotePosts = [];
      }
      const posts = mergeBlogPosts(embeddedPosts, remotePosts);
      const categories = [
        "All",
        ...[...new Set(posts.map((post) => post.category).filter(Boolean))].sort((a, b) =>
          a.localeCompare(b)
        )
      ];
      ...
      if (paramCategory) activeCategory = decodeURIComponent(paramCategory);
      if (paramQuery) {
        activeQuery = paramQuery.trim().toLowerCase();
        searchInput.value = paramQuery.trim();
      }
      ...
      const renderFilters = () => {
        filtersHolder.innerHTML = categories
          .map(
            (category) =>
              `<li><button class="cn-blog-category-link ${category === activeCategory ? "is-active" : ""}" data-category="${category}" type="button">${category}</button></li>`
          )
          .join("");
      };
      ...
      const setViewMode = (mode) => {
        ...
        if (defaultView) defaultView.hidden = isCategory;
        if (categoryView) categoryView.hidden = !isCategory;
      };

      const renderDefaultView = (filtered) => {
        ...
        featuredBlogList.innerHTML = featured.length
          ...
          : "<p class='cn-blog-empty'>No featured articles match the current filter.</p>";
        blogList.innerHTML = nonFeatured.length
          ? nonFeatured.map(card).join("")
          : "<p class='cn-blog-empty'>No articles match your search.</p>";
      };

      const renderCategoryView = (filtered) => {
        if (categoryTitle) categoryTitle.textContent = activeCategory;
        if (categoryDescription) categoryDescription.textContent = getBlogCategoryDescription(activeCategory);
        ...
        if (!filtered.length) {
          ...
          categoryList.innerHTML = `<p class="cn-blog-empty">${message}</p>`;
          return;
        }

        categoryList.innerHTML = filtered.map(card).join("");
      };

      const renderCards = () => {
        const filtered = posts.filter((post) => {
          const categoryMatch = activeCategory === "All" || postMatchesCategory(post, activeCategory);
          const queryMatch = postMatchesQuery(post, activeQuery);
          return categoryMatch && queryMatch;
        });

        if (activeCategory === "All") {
          setViewMode("default");
          renderDefaultView(filtered);
          return;
        }
      ...
      if (blogList) blogList.innerHTML = "<p class='cn-blog-empty'>Unable to load blog posts at this time.</p>";
      if (featuredBlogList) featuredBlogList.innerHTML = "";
      if (categoryList) {
        categoryList.innerHTML = "<p class='cn-blog-empty'>Unable to load blog posts at this time.</p>";
      }
    }
  };

  const initBlogArticleSidebar = async () => {
    const filtersHolder = document.getElementById("blogCategoryFilters");
    const searchInput = document.getElementById("blogSearchInput");
    if (!filtersHolder || !searchInput || document.getElementById("blogList")) return;
    ...
    try {
      ...
      const categories = [
        "All",
        ...[...new Set(remotePosts.map((post) => post.category).filter(Boolean))].sort((a, b) =>
          a.localeCompare(b)
        )
      ];
      ...
      filtersHolder.innerHTML = categories
        .map((category) => {
          ...
          return `<li><a class="cn-blog-category-link" href="${href}">${category}</a></li>`;
        })
        .join("");
      ...
      /* sidebar is decorative on article pages when data is unavailable */
    }
  };
  ...
  const renderToolsHub = async () => {
    ...
    if (!catalog) return;
    const hasStaticCards = catalog.querySelector("a.cn-tool-glass-card, a.cn-dashboard-micro-card");
    ...
    try {
      ...
      const toolAccent = (slug = "", navGroup = "") => {
        ...
        if (slug.includes("retirement") || slug.includes("take-home")) return "planning";
      ...
      const card = (tool) => {
        ...
        return `<a href="${tool.path}" class="cn-tool-glass-card cn-tool-glass-card--${accent}"><span class="cn-tool-glass-card__icon" aria-hidden="true">${svg}</span><span class="cn-tool-glass-card__title">${tool.name}</span></a>`;
      };
      const categoryTitle = (g) => {
        const svg = NAV_GROUP_SVG[g.key] || NAV_GROUP_SVG.lending;
        return `<h3 class="cn-tools-hub-category__title cn-tools-hub-category__title--${g.key}"><span class="cn-tools-hub-category__icon" aria-hidden="true">${svg}</span><span class="cn-tools-hub-category__label">${g.label} calculators</span></h3>`;
      };
      catalog.className = "cn-tools-dashboard cn-tools-hub-dashboard";
      catalog.innerHTML = `${GROUPS.filter((g) => buckets[g.key].length)
        .map(
          (g) => `<section class="cn-tools-dashboard__category">${categoryTitle(g)}<div class="cn-tools-hub-grid">${buckets[g.key].map(card).join("")}</div></section>`
        )
        .join("")}`;
    } catch (_e) {
      if (!hasStaticCards) {
