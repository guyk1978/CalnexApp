const GeoFinance = (() => {
  ...
  const renderSelectorInto = (host, idPrefix) => {
    ...
    wrap.innerHTML = `
      <label class="sr-only" for="${idPrefix}CountrySelect">Country</label>
      ...
      <select id="${idPrefix}CountrySelect" class="${selectClass}" aria-label="Country">
  ...
  const renderSelector = () => {
    ...
    if (!host) return;
    renderSelectorInto(host, "header");
  };

  const renderDashboardSelector = () => {
    if (document.body.dataset.page !== "dashboard") return;
    const anchor = document.getElementById("dashCountryMount");
    if (!anchor) return;
    renderSelectorInto(anchor, "dashboard");
  };
