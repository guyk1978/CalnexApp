const ScenarioUi = (() => {
  ...
  const updateActiveIndicator = () => {
    ...
    if (!shared.scenario) {
      setDerivedState({
        dashboard_active_scenario_label: "Active Scenario",
  ...
  const renderSavedScenarios = () => {
    ...
    if (!scenarios.length) {
      selectors.savedScenariosList.innerHTML = `<li class="cn-dash-saved-entry cn-dash-saved-entry--empty"><p class="muted">No saved scenarios yet. Build one and click Save Scenario.</p></li>`;
      return;
    }
    selectors.savedScenariosList.innerHTML = scenarios
      .map(
        (scenario) => `
          <li class="cn-dash-saved-entry" data-scenario-id="${scenario.id}">
            <p class="cn-dash-saved-entry__name">${scenario.name}</p>
            <p class="muted cn-dash-saved-entry__date">${formatDate(scenario.createdAt)}</p>
            <div class="cn-dash-saved-entry__actions scenario-actions">
