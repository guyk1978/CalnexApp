...
const CalnexCalculatorShareInit = (() => {
  ...
  const bindShareMenus = () => {
    ...
    if (document.documentElement.dataset.cnShareOutsideBound !== "1") {
      ...
      document.addEventListener("click", (event) => {
        if (event.target.closest("[data-cn-share-menu]")) return;
        closeAllShareMenus();
      });
  ...
  const init = () => {
    if (!window.CalnexCalculatorShare) return false;
  ...
  const scheduleInit = (attempt = 0) => {
    if (init()) return;
    if (attempt < 40) window.setTimeout(() => scheduleInit(attempt + 1), 50);
  };

  return { init, scheduleInit, refresh, showToast };
})();

window.CalnexCalculatorShareInit = CalnexCalculatorShareInit;
window.initShareButtons = () => CalnexCalculatorShareInit.scheduleInit();

document.addEventListener("DOMContentLoaded", () => {
  CalnexCalculatorShareInit.scheduleInit();
});
window.addEventListener("load", () => CalnexCalculatorShareInit.scheduleInit());
